...
export default function GiftCardsPage() {
  ...
  const exportCSV = () => {
    const csv = [
      ['Code', 'Montant initial', 'Montant restant', 'Destinataire', 'Email destinataire', 'Acheté par', 'Email acheteur', 'Date expiration', 'Statut'],
      ...giftCards.map(gc => [
        gc.code,
        Number(gc.initialAmount).toFixed(2),
        Number(gc.remainingAmount).toFixed(2),
        gc.recipientName || '',
        gc.recipientEmail || '',
        gc.purchasedByName || '',
        gc.purchasedByEmail || '',
        gc.expirationDate ? new Date(gc.expirationDate).toLocaleDateString('fr-FR') : '',
    ...
    a.href = url
    a.download = `bons_cadeaux_${new Date().toISOString()}.csv`
    a.click()
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Bons Cadeaux</h1>
        <div className="flex flex-wrap gap-2 w-full sm:w-auto">
          ...
          <button
            ...
            className="flex-1 sm:flex-none bg-blue-500 text-white px-4 py-2.5 rounded-lg hover:bg-blue-600 shadow-md transition-all duration-200 text-sm sm:text-base min-h-[44px]"
          >
            + Créer un bon cadeau
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <form onSubmit={handleSearch} className="flex gap-2">
            <input
              ...
              className="flex-1 border rounded-md px-3 py-2 dark:bg-gray-700 dark:text-gray-100 dark:border-gray-600"
            />
            <button
              type="submit"
              className="bg-gray-200 dark:bg-gray-600 dark:text-gray-100 px-4 py-2 rounded-md hover:bg-gray-300 dark:hover:bg-gray-500"
            >
              Rechercher
            </button>
          </form>
        ...
        <div className="overflow-x-auto">
          ...
          <thead className="bg-gray-50 dark:bg-gray-700">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Montant initial</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Montant restant</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Destinataire</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Acheté par</th>
                ...
                <tr key={card.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  ...
                  <td className="px-6 py-4 text-gray-900 dark:text-gray-100">
                    ...
                    {card.purchasedByEmail && (
                      <div className="text-xs text-gray-500 dark:text-gray-400">{card.purchasedByEmail}</div>
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-gray-900 dark:text-gray-100">
                    {card.expirationDate
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <div className="flex gap-2 items-center">
                      ...
                      <select
                        ...
                        className="border rounded px-2 py-1 text-xs dark:bg-gray-700 dark:text-gray-100 dark:border-gray-600"
                      >
